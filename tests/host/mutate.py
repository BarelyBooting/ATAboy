...
MUTANTS = [
    ...
    ('READ LONG: word 22 of 0 accepted', 'usb.c',
     '    if (!ide_read_long_word22(&w22) || w22 < 1 || w22 > IDE_LONG_ECC_MAX ||',
     '    if (!ide_read_long_word22(&w22) || w22 > IDE_LONG_ECC_MAX ||'),
    ('READ LONG: word 22 over 64 accepted', 'usb.c',
     '    if (!ide_read_long_word22(&w22) || w22 < 1 || w22 > IDE_LONG_ECC_MAX ||',
     '    if (!ide_read_long_word22(&w22) || w22 < 1 ||'),
    ('READ LONG: no IDENTIFY words needed', 'usb.c',
     '    if (!ide_read_long_word22(&w22) || w22 < 1 || w22 > IDE_LONG_ECC_MAX ||',
     '    if ((ide_read_long_word22(&w22), w22 = w22 ? w22 : 4, 0) || w22 < 1 || w22 > IDE_LONG_ECC_MAX ||'),
    ...
    ('READ LONG: more bytes than word 22 not noticed', 'ide.c',
     '                if (st & 0x08) goto long_bad_end;           // more bytes than word 22\n', ''),
    ('READ LONG: fewer bytes than word 22 not noticed', 'ide.c',
    ...
    ('READ LONG: ERR always resets', 'ide.c',
     '    if (!idle_after_error(2000)) soft_reset_restore();\n    return IDE_LONG_ERR;', '    soft_reset_restore();\n    return IDE_LONG_ERR;'),
    ...
    ('READ LONG: ide.c takes an ECC count of 0', 'ide.c',
     '    if (ecc < 1 || ecc > IDE_LONG_ECC_MAX) return IDE_LONG_NOT_SENT;', '    if (ecc > IDE_LONG_ECC_MAX) return IDE_LONG_NOT_SENT;'),
    ('READ LONG: ide.c takes an ECC count over 64', 'ide.c',
     '    if (ecc < 1 || ecc > IDE_LONG_ECC_MAX) return IDE_LONG_NOT_SENT;', '    if (ecc < 1) return IDE_LONG_NOT_SENT;'),
    # Not listed, equivalent: ide_read_long_word22's manual_chs_active test
    # and ide_manual_chs forgetting word 22 each stand in for the other (the
    # flag is only cleared by an IDENTIFY that answers, which sets word 22
    # again), and usb.c refuses Ctrl+G before either (5/24/00, tested).
