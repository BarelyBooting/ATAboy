...
MUTANTS = [
    ...
    ('no 400 ns wait after the READ command', 'ide.c',
     ...
     '    ide_write_reg(7, cmd);                                 // READ SECTORS EXT / READ SECTORS'),
    # The next four target the READ SECTORS path; READ LONG (0.6f3p9) has
    # its own copies of some of these lines, with mutants of their own below.
    ...
    ('stale DRQ recorded as a drive ERR', 'ide.c',
     'record_failure(IDE_FAIL_STALE_DRQ, 0, st, lba, 0, count);', 'record_failure(IDE_FAIL_ERR, 0, st, lba, 0, count);'),
    ...
    ('IORDY note: never on the picker', 'menus.c',
     '    if (iordy_note_due(!force_mode, id[49]))', '    if (0)'),
    ('IORDY note: shown on the F: Force picker (review LOW-4)', 'menus.c',
     '    if (iordy_note_due(!force_mode, id[49]))', '    if (iordy_note_due(true, id[49]))'),
    ...
    # --- 0.6f3p9, D: READ LONG (test_read) ---
    ('READ LONG: CORRCT accepted', 'usb.c',
     '    if (cdb[1] & 0x06) {', '    if (cdb[1] & 0x04) {'),
    ('READ LONG: PBLOCK accepted', 'usb.c',
     '    if (cdb[1] & 0x06) {', '    if (cdb[1] & 0x02) {'),
    ('READ LONG: sent after Ctrl+G (usb.c check gone)', 'usb.c',
     '    if (ide_manual_chs_active()) { tud_msc_set_sense(lun, SCSI_SENSE_ILLEGAL_REQUEST, 0x24, 0x00); return -1; }\n', ''),
    ('READ LONG: length from word 22 again (review M-1)', 'usb.c',
     '    uint32_t want = 512u + IDE_LONG_ECC_BYTES;', '    uint32_t want = 512u + w22;'),
    ('READ LONG: 8 vendor specific bytes, not the ATA-3 default 4', 'ide.h',
     '#define IDE_LONG_ECC_BYTES  4', '#define IDE_LONG_ECC_BYTES  8'),
    ('READ LONG: a data-out CBW accepted (review LOW-1)', 'usb.c',
     '    return img[12] == 0x80;', '    return true;'),
    ('READ LONG: CBW not checked at all (review LOW-1)', 'usb.c',
     '    if (!cbw_is_data_in(buf, lun, cdb, host_bufsize)) {', '    if (0) {'),
    ('READ LONG: CBW length not compared', 'usb.c',
     '    if ((uint16_t)len != host_bufsize) return false;\n    return img[12] == 0x80;', '    return img[12] == 0x80;'),
    ('READ LONG: ABRT reported as a medium error (review LOW-2)', 'ide.c',
     '    return last_fail.error == 0x04 ? IDE_LONG_ABRT : IDE_LONG_ERR;', '    return IDE_LONG_ERR;'),
    ('READ LONG: ABRT with other bits taken as not supported', 'ide.c',
     '    return last_fail.error == 0x04 ? IDE_LONG_ABRT : IDE_LONG_ERR;',
     '    return (last_fail.error & 0x04) ? IDE_LONG_ABRT : IDE_LONG_ERR;'),
    ('READ LONG: ABRT answered as NOT READY in usb.c', 'usb.c',
     '    case IDE_LONG_ABRT:     tud_msc_set_sense(lun, SCSI_SENSE_ILLEGAL_REQUEST, 0x20, 0x00); return -1;',
     '    case IDE_LONG_ABRT:     tud_msc_set_sense(lun, SCSI_SENSE_NOT_READY, 0x04, 0x00); return -1;'),
    ('READ LONG: no settle for DRQ after the last byte (review LOW-3)', 'ide.c',
     '#define IDE_LONG_SETTLE_US 1000', '#define IDE_LONG_SETTLE_US 0'),
    ('READ LONG: DRQ settle 100 ms', 'ide.c',
     '#define IDE_LONG_SETTLE_US 1000', '#define IDE_LONG_SETTLE_US 100000'),
    ('READ LONG: no IDENTIFY words needed', 'usb.c',
     '    if (!ide_read_long_word22(&w22) ||\n', '    if ((ide_read_long_word22(&w22), 0) ||\n'),
    ('READ LONG: a failed IDENTIFY keeps word 22', 'ide.c',
     '    long_words.valid = ok;\n', '    if (ok) long_words.valid = true;\n'),
    ('READ LONG: word 22 of another device taken', 'ide.c',
     '    if (!long_words.valid || long_words.dev_base != dev_base || manual_chs_active) return false;',
     '    if (!long_words.valid || manual_chs_active) return false;'),
    ('READ LONG: LBA48 mount accepted', 'usb.c',
     '        (config.use_lba_mode && config.lba_sectors > 0x0FFFFFFF)) {', '        0) {'),
    ('READ LONG: ide.c sends it on an LBA48 mount', 'ide.c',
     '    if (config.use_lba_mode && config.lba_sectors > 0x0FFFFFFF) return IDE_LONG_NOT_SENT;   // no 48-bit form\n', ''),
    ('READ LONG: length not checked', 'usb.c',
     '    if (len != want) {', '    if (0) {'),
    ('READ LONG: ILI information the wrong way round', 'usb.c',
     'len - want);   // two\'s complement if short', 'want - len);   // two\'s complement if short'),
    ('READ LONG: length refusal without ILI', 'usb.c',
     "        sense_ili_set(lun, SCSI_SENSE_ILLEGAL_REQUEST, 0x24, 0x00, len - want);",
     "        tud_msc_set_sense(lun, SCSI_SENSE_ILLEGAL_REQUEST, 0x24, 0x00);"),
    ('READ LONG: ILI added to a sense that is no longer ours', 'usb.c',
     '    if (!ours || bufsize < 18) return;', '    if (bufsize < 18) return;'),
    ('READ LONG: ILI pending not forgotten by the next command', 'usb.c',
     '    usb_sense_forget();                 // this command may set sense of its own\n', ''),
    ('READ LONG: zero length refused', 'usb.c',
     '    if (len == 0) return 0;\n', ''),
    ('READ LONG: host transfer length not checked', 'usb.c',
     '    if (host_bufsize < want) {', '    if (0) {'),
    ('READ LONG: LBA range not checked', 'usb.c',
     '    if ((uint64_t)lba >= total_sectors()) {', '    if (0) {'),
    ('READ LONG: last sector refused (> for >=)', 'usb.c',
     '    if ((uint64_t)lba >= total_sectors()) {', '    if ((uint64_t)lba + 1 >= total_sectors()) {'),
    ('READ LONG: sent while unmounted', 'usb.c',
     '    if (!is_mounted) { tud_msc_set_sense(lun, SCSI_SENSE_NOT_READY, 0x04, 0x00); return -1; }\n    if (cdb[1] & 0x06)',
     '    if (cdb[1] & 0x06)'),
    ('READ LONG: with retries (22h)', 'ide.c',
     '    const uint8_t cmd = 0x23;                               // READ LONG WITHOUT RETRIES',
     '    const uint8_t cmd = 0x22;                               // READ LONG WITHOUT RETRIES'),
    ('READ LONG: sector count is the ECC count', 'ide.c',
     '    if (config.use_lba_mode) {\n        ide_write_reg(2, 1);\n        ide_write_reg(3, lba & 0xFF);',
     '    if (config.use_lba_mode) {\n        ide_write_reg(2, ecc);\n        ide_write_reg(3, lba & 0xFF);'),
    ('READ LONG: CHS head from the wrong quotient', 'ide.c',
     '        uint8_t  head = tmp % config.heads;\n        uint16_t cyl  = tmp / config.heads;\n        ide_write_reg(2, 1);',
     '        uint8_t  head = lba % config.heads;\n        uint16_t cyl  = tmp / config.heads;\n        ide_write_reg(2, 1);'),
    ('READ LONG: CHS sector counted from 0', 'ide.c',
     '        uint8_t  sec  = (lba % config.spt) + 1;             // 1-based\n        uint8_t  head = tmp % config.heads;\n        uint16_t cyl  = tmp / config.heads;\n        ide_write_reg(2, 1);',
     '        uint8_t  sec  = (lba % config.spt);             // 1-based\n        uint8_t  head = tmp % config.heads;\n        uint16_t cyl  = tmp / config.heads;\n        ide_write_reg(2, 1);'),
    ('READ LONG: LBA bit not set', 'ide.c',
     '        ide_write_reg(5, (lba >> 16) & 0xFF);\n        ide_write_reg(6, (dev_base | 0x40) | ((lba >> 24) & 0x0F));\n    } else {\n        uint32_t tmp  = lba / config.spt;\n        uint8_t  sec  = (lba % config.spt) + 1;             // 1-based',
     '        ide_write_reg(5, (lba >> 16) & 0xFF);\n        ide_write_reg(6, dev_base | ((lba >> 24) & 0x0F));\n    } else {\n        uint32_t tmp  = lba / config.spt;\n        uint8_t  sec  = (lba % config.spt) + 1;             // 1-based'),
    ('READ LONG: lost CHS geometry not sent again', 'ide.c',
     '    if (!chs_geometry_ok()) {\n        record_failure(IDE_FAIL_NO_GEOMETRY, 0x91, ide_read_reg(7), lba, 0, 1);',
     '    if (0) {\n        record_failure(IDE_FAIL_NO_GEOMETRY, 0x91, ide_read_reg(7), lba, 0, 1);'),
    ('READ LONG: sent while a reset is pending', 'ide.c',
     '    if (!recovery_gate()) return IDE_LONG_NOT_SENT;', '    (void)recovery_gate();'),
    ('READ LONG: ECC bytes taken from bits 15:8', 'ide.c',
     '        if (out) out[i] = (uint8_t)(w & 0xFF);', '        if (out) out[i] = (uint8_t)(w >> 8);'),
    ('READ LONG: ECC bytes read as words, two per transfer', 'ide.c',
     '        if (out) out[i] = (uint8_t)(w & 0xFF);', '        if (out) { out[i] = (uint8_t)(w & 0xFF); if (i + 1 < n) out[++i] = (uint8_t)(w >> 8); }'),
    ('READ LONG: ECC bytes read without looking at DRQ', 'ide.c',
     '        if ((*st & 0x88) != 0x08) return i;             // BSY, or no DRQ: nothing more on offer\n', ''),
    ('READ LONG: data bytes swapped in each word', 'ide.c',
     '            buf[2 * i]     = (uint8_t)(words[i] & 0xFF);\n            buf[2 * i + 1] = (uint8_t)(words[i] >> 8);',
     '            buf[2 * i]     = (uint8_t)(words[i] >> 8);\n            buf[2 * i + 1] = (uint8_t)(words[i] & 0xFF);'),
    ('READ LONG: more than 4 bytes not noticed', 'ide.c',
     '                    if ((st & 0x88) == 0x08) goto long_bad_end;     // more than 4\n', ''),
    ('READ LONG: fewer than 4 bytes not noticed', 'ide.c',
     '    if (read_long_ecc(buf + 512, ecc, &st) < ecc) goto long_bad_end;   // fewer than 4',
     '    (void)read_long_ecc(buf + 512, ecc, &st);'),
    ('READ LONG: bad end never reset', 'ide.c',
     '    if ((st & 0x08) || !idle_after_error(2000)) soft_reset_restore();\n    return IDE_LONG_ABORTED;',
     '    return IDE_LONG_ABORTED;'),
    ('READ LONG: ERR not recorded', 'ide.c',
     '    record_failure(IDE_FAIL_ERR, cmd, st, lba, 0, 1);\n', ''),
    ('READ LONG: ERR always resets', 'ide.c',
     '    if (!idle_after_error(2000)) soft_reset_restore();\n    // ABRT and nothing else',
     '    soft_reset_restore();\n    // ABRT and nothing else'),
    ('READ LONG: ERR with data not drained', 'ide.c',
     '        ide_drain_sector();\n        (void)read_long_ecc(0, ecc, &st);\n', ''),
    ('READ LONG: ERR drain leaves the ECC bytes', 'ide.c',
     '        (void)read_long_ecc(0, ecc, &st);\n', ''),
    ('READ LONG: ERR data kept as a salvage capture', 'ide.c',
     '        ide_drain_sector();\n        (void)read_long_ecc(0, ecc, &st);\n',
     '        salvage_sector(cmd, st, last_fail.error, lba);\n        (void)read_long_ecc(0, ecc, &st);\n'),
    ('READ LONG: a timeout not reset', 'ide.c',
     '    record_failure(IDE_FAIL_TIMEOUT, cmd, st, lba, 0, 1);\n    soft_reset_restore();',
     '    record_failure(IDE_FAIL_TIMEOUT, cmd, st, lba, 0, 1);'),
    ('READ LONG: no time limit of its own (host budget ignored)', 'ide.c',
     '    uint32_t limit = work_ms(IDE_CMD_TIMEOUT_MS);           // or what the host command has left\n\n    for (;;) {',
     '    uint32_t limit = 30000;           // or what the host command has left\n\n    for (;;) {'),
    ('READ LONG: ERR taken as data (DRQ before ERR)', 'ide.c',
     '        if (st & 0x01) goto long_err;\n        if (st & 0x08) break;',
     '        if (st & 0x08) break;\n        if (st & 0x01) goto long_err;'),
    ('READ LONG: ERR reported as NOT READY', 'usb.c',
     '    case IDE_LONG_ERR:      tud_msc_set_sense(lun, SCSI_SENSE_MEDIUM_ERROR, 0x11, 0x00); return -1;',
     '    case IDE_LONG_ERR:      tud_msc_set_sense(lun, SCSI_SENSE_NOT_READY, 0x04, 0x00); return -1;'),
    ('READ LONG: returns 512 bytes only', 'usb.c',
     '    case IDE_LONG_OK:       return (int32_t)want;', '    case IDE_LONG_OK:       return 512;'),
    ('READ LONG: not marked busy for core 1', 'usb.c',
     '        msc_busy_begin();\n        host_cmd_enter(false, false, 0);        // one callback, one command\n        int32_t r = read_long10(',
     '        host_cmd_enter(false, false, 0);        // one callback, one command\n        int32_t r = read_long10('),
    ('READ LONG: unmount keeps word 22', 'menus.c',
     '    ide_read_long_forget();         // word 22 (READ LONG), kept in every build\n', ''),
    ('READ LONG: a probe keeps word 22', 'ide.c',
