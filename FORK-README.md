...
16. **Sector reads and writes wait by the clock, and for as long as the host will.** Found on real hardware in 0.6f3p6: a Seagate alone on the cable as a slave took longer than usual to finish one single-sector write, and the firmware reset it in the middle of it. The wait after a write counted polls, about a second's worth, instead of time. The sector had been written; the drive just hadn't said so yet. After that reset the drive stayed busy and then stopped answering altogether until the next detect. Every wait on a read, a write or IDENTIFY is now against the clock. A read or write gets 30 seconds from the command to the end, the whole command and not each sector, which is what Linux gives a disk command before it gives up (Windows uses 10 seconds unless the registry says otherwise, often 60). IDENTIFY gets 10 seconds.

17. **A drive that doesn't come back from a soft reset gets one hardware reset.** The firmware now pulses the IDE reset line, the same one Auto Detect uses, then selects the drive again, waits up to the 31 seconds ATA allows, sends RECALIBRATE and puts the CHS geometry back. It only tries once. The reset line resets both drives on the cable. Debug E shows "HW reset" when this was needed, or "HW reset FAILED" when even that didn't bring the drive back.
...
19. **Auto Detect and Debug Mode wait for a USB command that's still running.** After an unmount, a slow command that started before it can still be using the drive. Detect and the debug keys now wait for it (up to a minute, Esc cancels) instead of using the IDE bus at the same time, and do nothing if it doesn't finish. A build with `-DATABOY_SAT_SMART_SAVES=1` now calls itself `0.6f3p7-palimpsest-smartsaves` and shows `(fork+smartsaves)` in the setup screen, so it can't be mistaken for the normal build.

The fork's build calls itself `0.6f3p7-palimpsest` and shows `v0.6f3p7 (fork)` in the setup screen (0.6f3p1 before items 6 to 15, 0.6f3p6 before 16 to 19), so it can't be mistaken for a stock v0.6f3.
